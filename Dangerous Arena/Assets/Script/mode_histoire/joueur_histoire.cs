...
public class joueur_histoire : MonoBehaviour
{
    ...
    private int pointToHard, pointToDivin = 1000;
    private GameObject m_barre_de_vie;
    private int distance_parcouru = 0;
    private int ancienn_distance = 0;
    private int derniere_generation = 0;
    ...
    public GameObject sol_normal;
    public GameObject derniere_generation_spawn;
    ...
    private void Awake()
    {
        ...
        derniere_generation_spawn = sol_normal;
    ...
    private void Update()
    {
        distance_parcouru = (int)transform.position.x;
        if (ancienn_distance + 10 == distance_parcouru)
        {
            ancienn_distance = distance_parcouru;
            ajouterPoint(10);
        }
        if (ancienn_distance > 40 && ancienn_distance >= derniere_generation)
        {
            //Debug.Log("generation");
            derniere_generation = ancienn_distance + 20;
            derniere_generation_spawn = Instantiate(sol_normal, new Vector3(derniere_generation_spawn.transform.position.x + 22, 0, 0), Quaternion.identity);
        }

        //Debug.Log(distance_parcouru);
    ...
    public void ajouterPoint(int point)
    {
        ...
        if (m_nb_point >= 1000 && !m_mode_hardcore)
        {
            ...
            GameObject.Find("Spawner").GetComponent<spawner_histoire>().modeHardcore();
        ...
        if (m_nb_point >= 2000 && !m_mode_divin)
        {
            ...
            GameObject.Find("Spawner").GetComponent<spawner_histoire>().modeDivin();
            GameObject.Find("son").GetComponent<AudioSource>().Stop();

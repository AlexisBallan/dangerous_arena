...
public class Ennemi_IA_hardcore : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (!m_estMort)
        {
            ...
            {
                collision.gameObject.GetComponent<joueur_hardcore>().subirDegat(nombre_degats);
            }
        }
    }
    ...
    public void enleverPointDeVie()
    {
        ...
        if (point_de_vie <= 0 && !m_estMort)
        {
            ...
            GameObject.Find("perso").GetComponent<joueur_hardcore>().ajouterPoint(nombre_point);
            StartCoroutine(mort());
        }
    }

...
public class joueur_hardcore : MonoBehaviour
{
    ...
    public void ajouterPoint(int point)
    {
        m_nb_point += point;
        m_text_point.text = "Points : " + m_nb_point;
        if (m_nb_point >= 1 && !m_mode_hardcore)
        {
            m_mode_hardcore = true;
            mutliplicateur_point++;
            modeHardcoreCanvas.SetActive(true);
            GameObject.Find("Spawner").GetComponent<Spawner_hardcore>().modeHardcore();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        switch (collision.gameObject.tag)
        {
            ...
            case "multidirectionnel":
                ...
                break;
            case "gachette":
                if (tire_enfoncer_temps <= 0.1f)
                    ajouterPoint(5 * mutliplicateur_point);
                else
                    tire_enfoncer_temps -= .02f;
                bonus_gachette++;
